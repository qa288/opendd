...
const BASE_DIR = '/home/node/.openclaw/mcp/lark-openapi';
...
function defaultTarget() {
  const path = '/home/node/.openclaw/credentials/feishu-default-allowFrom.json';
...
async function main() {
  ...
  const target = arg('--target') || process.env.FEISHU_AUTH_TARGET || defaultTarget();
  const publicUrl = (arg('--public-url') || process.env.LARK_MCP_PUBLIC_URL || 'https://ope.tyos.cc').replace(/\/$/, '');
  ...
  const scopeText = process.env.LARK_MCP_SCOPE || 'auth:user.id:read contact:user.base:readonly contact:user.basic_profile:readonly contact:contact.base:readonly wiki:space:retrieve wiki:node:read wiki:node:retrieve space:document:retrieve docx:document:readonly sheets:spreadsheet.meta:read sheets:spreadsheet:read base:app:read base:table:read base:field:read base:view:read base:record:retrieve search:docs:read im:chat:read im:chat.members:read im:message:readonly im:message.group_msg:get_as_user im:message.p2p_msg:get_as_user search:message';

  if (!appId || !appSecret || typeof appSecret !== 'string') {
    throw new Error('Feishu appId/appSecret not found in OpenClaw config.');
  }
  if (!target) {
    throw new Error('Missing target. Pass --target <open_id|chat_id> or configure feishu-default-allowFrom.json.');
  }
